...
class FiltrateAlerView: UIView {
    ...
    open lazy var indicator:JXSegmentedIndicatorLineView = {
        let indicator = JXSegmentedIndicatorLineView.init(frame: CGRect.init(x: 0, y: 0, width: kRPRealValue(25), height: kRPRealValue(5)))
//        indicator.indicatorColor = UIColor.COLORF52938
        indicator.lineStyle = .lengthen
        indicator.indicatorWidthIncrement = kRPRealValue(-5)
        indicator.indicatorHeight = kRPRealValue(5)
        return indicator
    }()
    ...
    private var _typeHeight:CGFloat = kRPRealValue(69)
    ...
    open lazy var entryTagView : EntryTagView = {
        ...
        entryTagView.font = kRPRealValue(14)
        entryTagView.flowLayout.itemSize = CGSize.init(width: kRPRealValue(105), height: kRPRealValue(30))
    ...
    open func setupUI(filTypeAler: FilTypeAler) {
        ...
        case .time:
            ...
            entryTagView.titleL.snp.updateConstraints { make in
                make.height.equalTo(kRPRealValue(30))
            }
        ...
        case .priceMore:
            ...
            segmentBar.snp.makeConstraints { (make) in
                make.centerX.equalToSuperview().offset(kRPRealValue(0))
                make.width.equalTo(kRPRealValue(250))
                make.top.equalToSuperview().offset(0)
                make.height.equalTo(kRPRealValue(45)).priority(100)
            }
            segmentedDataSource.titles =  ["单价","总价"]
            segmentedDataSource.reloadData(selectedIndex: 0)
            segmentedDataSource.titleNormalColor = .COLOR252525
            segmentedDataSource.titleSelectedColor = .COLORF52938
            segmentedDataSource.isTitleColorGradientEnabled = true
            segmentedDataSource .titleNormalFont = .systemFont(ofSize: kRPRealValue(14))
            segmentedDataSource.titleSelectedFont = .boldSystemFont(ofSize: kRPRealValue(17))
            segmentedDataSource.itemSpacing = kRPRealValue(60)
            unowned let uSelf = self
        ...
        case .threeRegion:
            ...
            JuAddThreeFiltrate.shared.filtrateTabview.snp.remakeConstraints { make in
                ...
                make.height.equalTo(kRPRealValue(193))
            }
            ...
            JuAddThreeFiltrate.shared.footV.snp.remakeConstraints { make in
                ...
                make.height.equalTo(kRPRealValue(69))
            }
            break
   ...
   open func filtrateAlerViewHeight() -> CGFloat {
        ...
        case .time:
            ...
            let  s =  Int (JSCREENW - kRPRealValue(30)) / Int (entryModel.itemSize?.width + kRPRealValue(10) ?? 0)
            var n:Int = (entryModel.groups?.count ?? 0) / s
            if (entryModel.groups?.count ?? 0) % s > 0{
                n  += 1
            }
            return (entryModel.itemSize?.height ?? 0 + kRPRealValue(5)) * CGFloat(n) + kRPRealValue(40) + kRPRealValue(40) + typeHeight + kRPRealValue(45)
        case .region:
            return kRPRealValue(262)
        case .price:
            let entryModel = groups.firstObject as? EntryModel  ?? EntryModel.init()
            entryTagView.titleL.text = entryModel.title
            let  s =  Int (JSCREENW - kRPRealValue(30)) / Int (entryModel.itemSize?.width + kRPRealValue(10) ?? 0)
            ...
            return (entryModel.itemSize?.height ?? 0 + kRPRealValue(5)) * CGFloat(n) + kRPRealValue(40) + kRPRealValue(40) + typeHeight
        ...
        case .more:
            return itemSizeArrayHeight()
        case .sort:
            return CGFloat(array.count ) * kRPRealValue(35) + typeHeight
        case .none:
            return itemSizeArrayHeight()
        case .threeRegion:
            return kRPRealValue(262)
    ...
    func itemSizeArrayHeight() -> CGFloat {
        ...
        array.enumerateObjects { object, index, stop in
            ...
            let  s =  Int (JSCREENW - kRPRealValue(30)) / Int (obj.itemSize?.width + kRPRealValue(5) ?? 0)
            let itemHeight = (obj.itemSize?.width ?? 0 > kRPRealValue(105) ?  ((obj.itemSize?.height ?? 0) + kRPRealValue(6)) : ( (obj.itemSize?.height ?? 0) + kRPRealValue(20)))
            if obj.title?.length ?? 0 > 0 {
                height +=  itemHeight *  ((CGFloat((obj.groups?.count ?? 1)) / CGFloat(s)) + 1) + ((CGFloat((obj.groups?.count ?? 1)) / CGFloat(s)) > 1 ?  0 : kRPRealValue(15))
            }else{
                height +=  itemHeight *  ((CGFloat((obj.groups?.count ?? 1)) / CGFloat(s)) > 1 ? ((CGFloat((obj.groups?.count ?? 1)) / CGFloat(s)) + 1) : 1)
            }
            if obj.type == .image  {
                height += kRPRealValue(40)
            }
            if obj.type == .images {
                height += kRPRealValue(40)
            }
            if obj.type == .three {
                height += kRPRealValue(70)
            }
        }
        height +=  kRPRealValue(70)
        return height
    ...
    override func layoutSubviews() {
        ...
        case .time:
            ...
            calendarV.snp.remakeConstraints { make in
                ...
                make.bottom.equalTo(filtrateBotView.snp_top).offset(-kRPRealValue(5))
            }
            
            entryTagView.snp.remakeConstraints { make in
                make.top.equalToSuperview().offset(kRPRealValue(5))
                make.left.right.equalToSuperview().offset(0)
                make.bottom.equalTo(priceView.snp_top).offset(-kRPRealValue(5))
            }
        ...
        case .price:
            ...
            entryTagView.titleL.snp.remakeConstraints{ make in
               ...
               if entryTagView.titleL.text?.length ?? 0 > 0{
                   make.top.equalToSuperview().offset(kRPRealValue(15))
                    make.height.equalTo(kRPRealValue(30))
               }else{
                   make.height.equalTo(kRPRealValue(0))
                   make.top.equalToSuperview().offset(kRPRealValue(0))
               }
            }
        ...
        case .priceMore:
            ...
            entryTagView.snp.remakeConstraints { make in
                make.top.equalToSuperview().offset(kRPRealValue(45))
                make.left.right.equalToSuperview().offset(0)
                make.bottom.equalTo(priceView.snp_top).offset(-kRPRealValue(5))
            }
            entryTagView.titleL.snp.remakeConstraints{ make in
